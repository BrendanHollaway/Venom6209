...
public class NewRobotics {
    ...
    {
        return calculate_heading(center, 0);
    }
    public static double calculate_heading(DPoint center, double time) //encoder represents distance travelled. max_encoder is target distance
    {
        if(time > 3)
            return 0;
        double ratio = 1.0 / 1.32; // width of view divided by distance to object- test with ruler
        int width_pixels = 480; // width of screen
        double depth_inches = 18 * (1 - (time / 4));
